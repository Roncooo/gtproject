...
from utilities.utils import NUM_CARDS_PER_PLAYER, LOWEST_CARD, HIGHEST_CARD, NUMBER_OF_CARDS, show_visible_cards
...
if __name__ == "__main__":
    ...
    for i, path in enumerate(all_paths):
        ...
        for node in path[1:]:
            table.add_row([node.parent.current_player, node.card_just_played, show_visible_cards(node.visible_cards),
                           node.delta_score, node.cards_player1, node.cards_player2])
    print(table)